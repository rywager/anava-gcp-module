...
electron_1.ipcMain.handle('discover-cameras', async () => {
    const { discoverCameras } = await Promise.resolve().then(() => __importStar(__webpack_require__(/*! ../services/cameraDiscovery */ "./src/services/cameraDiscovery.ts")));
    return discoverCameras();
});
electron_1.ipcMain.handle('deploy-acap', async (event, cameraIP, config) => {
    const { deployACAP } = await Promise.resolve().then(() => __importStar(__webpack_require__(/*! ../services/acapDeployment */ "./src/services/acapDeployment.ts")));
    return deployACAP(cameraIP, config);
});
electron_1.ipcMain.handle('get-cloud-config', async () => {
    const { fetchCloudConfig } = await Promise.resolve().then(() => __importStar(__webpack_require__(/*! ../services/cloudConfig */ "./src/services/cloudConfig.ts")));
...
/***/ "./src/services/acapDeployment.ts":
/*!****************************************!*\
  !*** ./src/services/acapDeployment.ts ***!
  \****************************************/
/***/ (function(__unused_webpack_module, exports, __webpack_require__) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.deploymentEmitter = void 0;
exports.deployACAP = deployACAP;
exports.batchDeploy = batchDeploy;
...
exports.deploymentEmitter = new events_1.EventEmitter();
async function deployACAP(cameraIP, config) {
...
async function downloadACAP(url) {
...
async function uploadACAP(cameraIP, credentials, acapPath) {
    const fileData = await fs.readFile(acapPath);
    const formData = new FormData();
    formData.append('file', new Blob([fileData]), 'BatonDescribe.eap');
    const response = await axios_1.default.post(`http://${cameraIP}${VAPIX_UPLOAD}`, formData, {
        auth: credentials,
        headers: {
            'Content-Type': 'multipart/form-data'
        }
...
async function installACAP(cameraIP, credentials, packageName) {
...
async function controlACAP(cameraIP, credentials, packageName, action) {
    const response = await axios_1.default.post(`http://${cameraIP}${VAPIX_INSTALL}`, `package=${packageName}&action=${action}`, {
        auth: credentials,
        headers: {
...
async function listApplications(cameraIP, credentials) {
    const response = await axios_1.default.get(`http://${cameraIP}${VAPIX_LIST}`, { auth: credentials });
...
async function configureACAP(cameraIP, credentials, config) {
...
function emitProgress(cameraId, stage, progress, message, error) {
    const progressData = {
        cameraId,
        stage,
        progress,
    ...
    exports.deploymentEmitter.emit('progress', progressData);
}
async function batchDeploy(cameras, config) {
...
async function checkCamera(ip, username, password) {
    try {
        // Try to get device info via VAPIX
        const response = await axios_1.default.get(`http://${ip}${VAPIX_DEVICE_INFO}`, {
            auth: {
                username,
                password
            },
...
async function getCameraDetails(ip, credentials) {
    try {
        const response = await axios_1.default.get(`http://${ip}${VAPIX_PARAMS}`, {
            auth: credentials,
            timeout: 5000
        });
        // Parse parameters
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.fetchCloudConfig = fetchCloudConfig;
exports.saveDeploymentId = saveDeploymentId;
exports.getAvailableDeployments = getAvailableDeployments;
const axios_1 = __importDefault(__webpack_require__(/*! axios */ "./node_modules/axios/dist/node/axios.cjs"));
const terraformIntegration_1 = __webpack_require__(/*! ./terraformIntegration */ "./src/services/terraformIntegration.ts");
...
async function fetchCloudConfig() {
    try {
        ...
        const terraformConfig = await (0, terraformIntegration_1.loadTerraformOutputs)();
        if (terraformConfig) {
        ...
        const response = await axios_1.default.get(`https://terraform-installer-api.run.app/api/deployment/${deploymentId}/acap-config`);
...
function parseToCloudConfig(outputs) {
    return {
...
async function saveDeploymentId(deploymentId) {
...
async function getAvailableDeployments() {
    try {
        const response = await axios_1.default.get('https://terraform-installer-api.run.app/api/deployments');
        return response.data;
    }
    catch (error) {
...
/***/ "./src/services/terraformIntegration.ts":
/*!**********************************************!*\
  !*** ./src/services/terraformIntegration.ts ***!
  \**********************************************/
/***/ (function(__unused_webpack_module, exports, __webpack_require__) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.loadTerraformOutputs = loadTerraformOutputs;
exports.saveTerraformOutputs = saveTerraformOutputs;
exports.watchTerraformOutputs = watchTerraformOutputs;
const fs = __importStar(__webpack_require__(/*! fs/promises */ "fs/promises"));
const path = __importStar(__webpack_require__(/*! path */ "path"));
...
function parseOutputsToConfig(outputs) {
...
function parseStateToConfig(state) {
    // Extract outputs from terraform state
    const outputs = state.outputs || {};
    return parseOutputsToConfig(outputs);
}
async function saveTerraformOutputs(config) {
    try {
...
async function watchTerraformOutputs(callback) {
    const outputPath = path.join(__dirname, '../../../outputs.json');
    try {
        // Check if file exists and watch for changes
        await fs.access(outputPath);
        // Use fs.watch to monitor file changes
        const { watch } = await Promise.resolve().then(() => __importStar(__webpack_require__(/*! fs */ "fs")));
