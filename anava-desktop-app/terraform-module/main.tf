...
# Try to use existing bucket first, create if it doesn't exist
resource "google_storage_bucket" "firebase_bucket" {
  ...
  lifecycle {
    ignore_changes = [name, project, project_number, self_link, url, labels, effective_labels, terraform_labels, soft_delete_policy]
...
resource "google_storage_bucket" "function_source" {
  ...
  lifecycle {
    ignore_changes = [project, project_number, self_link, url, labels, effective_labels, terraform_labels, soft_delete_policy]
  }
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      if gcloud api-gateway apis describe "${var.solution_prefix}-api" --project="${var.project_id}" &>/dev/null; then
        ...
        # List and delete any existing gateways (more resilient approach)
        GATEWAYS=$(gcloud api-gateway gateways list --project="${var.project_id}" --format="value(name)" --filter="api.name:${var.solution_prefix}-api OR displayName:${var.solution_prefix}" 2>/dev/null || echo "")
        if [ -n "$GATEWAYS" ]; then
          echo "Deleting existing gateways..."
          for gw in $GATEWAYS; do
            echo "Attempting to delete gateway: $gw"
            gcloud api-gateway gateways delete "$gw" --location="${var.region}" --project="${var.project_id}" --quiet 2>/dev/null || echo "Could not delete $gw (may not exist or already deleted)"
          done
          echo "Waiting for gateway deletions to complete..."
          sleep 30
        fi
        
        # Delete API configs first
        echo "Deleting API configs..."
        CONFIGS=$(gcloud api-gateway api-configs list --api="${var.solution_prefix}-api" --project="${var.project_id}" --format="value(name)" 2>/dev/null || echo "")
        if [ -n "$CONFIGS" ]; then
          for config in $CONFIGS; do
            echo "Attempting to delete config: $config"
            gcloud api-gateway api-configs delete "$config" --api="${var.solution_prefix}-api" --project="${var.project_id}" --quiet 2>/dev/null || echo "Could not delete config $config"
          done
          echo "Waiting for config deletions to complete..."
          sleep 30
        fi
        
        # Delete the API
        echo "Deleting existing API..."
        gcloud api-gateway apis delete "${var.solution_prefix}-api" --project="${var.project_id}" --quiet 2>/dev/null || echo "Could not delete API (may not exist or already deleted)"
        
        # Wait for deletion to propagate
        echo "Waiting for API deletion to complete..."
        sleep 60
      else
